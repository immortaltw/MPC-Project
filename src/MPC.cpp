...
public:
  ...
  void SetVals(double steer, double throttle) { steer_val_ = steer; throttle_ = throttle; }
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    double w_epsi = 2000;
    double w_v = 0.1;
    double w_delta = 100000;
    double w_a = .01;
    double w_delta_diff = 100000;
    double w_a_diff = 10;
    ...
    for (int t = 1; t < N; ++t) {
      ...
      // Consider the actuation w.r.t system delay.
      // System delay is 0.1 secs, our dt is 0.05, meaning
      // that we should delay the actuation by 2 epoches.
      AD<double> delta0 = 0;
      AD<double> a0 = 0;
      if (t > 2) {
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  fg_eval.SetVals(steer_val, throttle);
